...
router.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const imageFilename = `${timestamp}-${file.filename}${path.extname(file.originalname)}`;
    const imagePath = path.join(UPLOADS_DIR, imageFilename);
    const thumbnailFilename = `thumb-${timestamp}-${file.filename}.png`;
    const thumbnailPath = path.join(UPLOADS_DIR, thumbnailFilename);

    // Determine result state
    const extractionSuccess = !!qrResult.data;
    const status = extractionSuccess ? 'processed' : 'failed';
    const errorMessage = extractionSuccess ? null : qrResult.error || 'No text detected';
    ...
    res.status(200).json({
      ...
      extractionSuccess: saved.extraction_success,
      quality: saved.quality,
      processedAt: saved.created_at,
  ...
  try {
    ...
    const submissionsResult = await pool.query(
      `SELECT
         ...
         extraction_success  AS "extractionSuccess",
         status,
         quality,
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM image_submissions');
    const total = parseInt(countResult.rows[0].count, 10);

    // Expose just the thumbnail filename (like previous behavior)
    const submissions = submissionsResult.rows.map(row => ({
      ...row,
      thumbnailUrl: row.thumbnailPath ? path.basename(row.thumbnailPath) : null,
    }));
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...row,
      thumbnailUrl: row.thumbnailPath ? path.basename(row.thumbnailPath) : null,
      originalFilename: row.originalImagePath ? path.basename(row.originalImagePath) : null,
    });
  } catch (err) {
    console.error(err);
